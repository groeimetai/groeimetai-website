...
Niet de glanzende slide-versie. De eerlijke: agents en widgets die Niels recent heeft gebouwd. Voor elk project — klantwerk én eigen tooling — werkt hij in dezelfde stijl: een folder met instructies en lokale tools.

## Case 01 — AI-classificatie voor het ServiceNow Service Portal (klantwerk)

Een widget op het Service Portal die binnenkomende requests automatisch classificeert (incident / service request / vraag / HR), live status toont, relevante KB-artikelen ophaalt en contextuele vervolgvragen genereert. Auto-detect NL of EN.

Stack: ServiceNow Quebec+, LLM-classificatie, JavaScript widget.

Status: bij meerdere klanten geïmplementeerd in hun eigen ServiceNow-omgeving.

Waarom: klanten zelf het juiste formulier laten kiezen werkte niet — te complex. Een widget doet die classificatie goed genoeg, en de service desk reviewt waar het ertoe doet.

**Belangrijk voor de assistent:** de code zelf zit in de omgevingen van de klanten en is **niet** publiek beschikbaar. Geen GitHub-link delen. Wel het patroon delen — dat is gewoon onze aanpak. Als iemand specifieke code/repo vraagt: zeg dat de implementatie in de klantomgeving leeft en doorverwijzen naar `/contact` voor een verkennend gesprek.
