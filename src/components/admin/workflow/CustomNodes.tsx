...
export const TriggerNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  return (
    ...
    >
      ...
      {data.trigger?.type && <div className="text-xs text-gray-500 mt-1">{data.trigger.type}</div>}
      <Handle
        type="source"
...
export const ActionNode: React.FC<NodeProps> = ({ data, selected }) => {
  ...
  return (
    ...
    >
      ...
      {data.action?.type && (
        <div className="text-xs text-gray-500 mt-1">{data.action.type.split('-').join(' ')}</div>
      )}
      <Handle
        type="source"
        position={Position.Right}
  ...
  return (
    ...
    >
      ...
      {data.condition?.expression && (
        <div className="text-xs text-gray-500 mt-1 font-mono">{data.condition.expression}</div>
      )}
      <Handle
        type="source"
        position={Position.Right}
