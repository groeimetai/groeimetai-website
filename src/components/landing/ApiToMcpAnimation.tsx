...
function radialPositions(count: number, radiusPct = 40, startDeg = -90, isMobile = false) {
  if (isMobile) {
    // Mobile: simplified to single API positioned at top with better spacing
    return [
      { top: 15, left: 50 }   // Single API - better top spacing for professional layout
    ];
  } else {
...
function betweenCenter(pos: { top: number; left: number }, f = 0.6, isMobile = false) {
  if (isMobile) {
    // Mobile: single MCP positioned in middle for clean vertical flow with better spacing
    return { top: 45, left: 50 }; // Fixed position between API (15%) and AI (75%) - professional spacing
  } else {
          ...
          <div className="bg-white/5 backdrop-blur-sm border border-white/10 rounded-2xl p-6 lg:p-10 mb-12">
            ...
            >
              ...
              >
                ...
                {showLinesMCPAgent &&
                  (isMobile ? mcpPositions.slice(0, 1) : mcpPositions).map((p, i) => {
                    ...
                    // Agent box center
                    const c = getBoxCenter({ top: isMobile ? 75 : 55, left: 50 }, stageSize); // Agent connection point (mobile lower with better spacing, desktop original)
                    return (
                      <motion.line
                        key={`m-c-${i}`}
                        x1={m.x}
              ...
              <div
                ...
                style={{ top: isMobile ? '75%' : '55%', left: '50%', zIndex: 35 }}
              >
                <motion.div
